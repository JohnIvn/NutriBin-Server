    ...
    <div className="flex justify-center items-center w-screen h-screen fixed inset-0 z-[100] backdrop-blur-md bg-black/20 p-4">
      <Card className="w-full max-w-md shadow-2xl border-none animate-in fade-in zoom-in duration-200">
        <CardHeader className="flex flex-col items-center gap-4 pt-8">
          <div className={`p-4 rounded-full ${styles.bg}`}>
            {styles.icon}
          </div>
        ...
        <CardFooter className="flex flex-col gap-3 w-full pb-8 px-8">
          <Button 
            type="button" 
            onClick={confirm} 
            className={`${styles.btn} text-white h-12 text-lg font-semibold w-full transition-all active:scale-95 cursor-pointer shadow-lg`}
          >
            {mode || 'Confirm'}
          </Button>
          ...
          <Button 
            ...
            onClick={cancel} 
            variant="ghost" 
            className="w-full h-12 text-gray-500 hover:bg-gray-100 cursor-pointer"
          >
            Go Back
          </Button>
        </CardFooter>
